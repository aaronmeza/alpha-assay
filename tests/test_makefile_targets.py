...
def test_make_sample_produces_fixture():
    ...
    if fixture.exists():
        fixture.unlink()
    result = subprocess.run(["make", "sample"], cwd=ROOT, capture_output=True, text=True, timeout=60)
    ...
    row_count = len(rows)
    assert 300 <= row_count <= 2000, f"row count {row_count} outside expected band [300, 2000] for --days 2"
